...
@section('content')
    {!! Form::model($user,['route' => ['backend.access.users.update', $user->id], 'class' => 'form-horizontal', 'role' => 'form', 'method' => 'PATCH']) !!}
                    ...
                    <div class="col-lg-3">
                        @if (count($roles) > 0)
                            @foreach($roles as $role)
                                <input type="checkbox" value="{{$role->id}}"  name="assignees_roles[]" id="role-{{$role->id}}" {{in_array($role->id, $user_roles) ? 'checked' : ''}} /> <label for="role-{{$role->id}}">{!! $role->name !!}</label>
                                ...
                                <div class="col-lg-3">
                                    <ul style="margin:0;padding:0;list-style:none;">
                                        @foreach ($perm as $p)
                                            ...
                                            <li><input type="checkbox" value="{{$p['id']}}" name="permission_user[]" {{in_array($p['id'], $user_permissions) ? 'checked' : ""}} data-dependencies="{!! $dependencies !!}" id="permission-{{$p['id']}}"> <label for="permission-{{$p['id']}}" />
    ...
    <div class="box box-info">
        <div class="box-body">
            <div class="pull-left">
                <a href="{{route('backend.access.users.index')}}" class="btn btn-danger btn-xs">{{ trans('buttons.general.cancel') }}</a>
            </div>
